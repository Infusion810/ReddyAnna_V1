...
const TournamentWinner = ({
  ...
  const formattedData = React.useMemo(() => {
    if (!matchData?.runners) return data;

    return matchData.runners.map(runner => {
      // Get the first back and lay prices
      const backPrice = runner.back?.[0]?.price || 0;
      const layPrice = runner.lay?.[0]?.price || 0;
      
      // Convert to the format expected by the component
      return [
        runner.name, // Team name
        [
          (backPrice * 100).toFixed(2), // Lgaai odds (back)
          (backPrice * 100).toFixed(2)
        ],
        [
          (layPrice * 100).toFixed(2), // Khaai odds (lay)
          (layPrice * 100).toFixed(2)
        ]
      ];
    });
  }, [matchData, data]);
  // console.log("Data tournament winner", data);

  // Use formattedData instead of data prop
  const displayData = title === "Match Odds" ? formattedData : data;

  // console.log("matchData1", matchData);

  // Determine styling based on the data structure
  const tableStyle = "odds-row-style";

  // console.log("tselectedBet", tselectedBet);
  ...
  const handleBetSelect = (rowIndex, type, odds, runs) => {
    ...
    // console.log("data",data[rowIndex][1]);
  ...
  return (
    <div className={`tournament_winner ${title.toLowerCase() === 'bookmaker' ? 'bookmaker' : ''}`}>
      <div className={`T20_header ${title.toLowerCase() === 'bookmaker' ? 'bookmaker' : ''}`}>
        <h1>{title}</h1>
      </div>
        ...
        <Table>
          <thead>
            <tr>
              ...
              <TableHeader>{isMatchOdds || title === 'BOOKMAKER' ? "BACK" : "NO"}</TableHeader>
              <TableHeader>{isMatchOdds || title === 'BOOKMAKER' ? "LAY" : "YES"}</TableHeader>
            </tr>
          </thead>
          <tbody>
            {displayData
              ...
              .map((row, rowIndex) => {
                ...
                const isMatchOdds = title.toLowerCase().includes("match odds") || title === 'BOOKMAKER';
                const isOverMarket = title.toLowerCase().includes("over market");
                    ...
                    <TableCell>
                      <LagaiButton
                        className={`${isSelected && tselectedBet.type === (isMatchOdds ? "Lgaai" : "no") ? 'selected' : ''} ${
                          !row[1] || !row[1][0] || row[1][0] === '-' || isNaN(parseFloat(row[1][0])) ? 'suspended' : ''
                        }`}
                        onClick={() => {
                          if (!row[1] || !row[1][0] || row[1][0] === '-' || isNaN(parseFloat(row[1][0]))) return;
                          handleBetSelect(
                            rowIndex,
                      ...
                      >
                        {!row[1] || !row[1][0] || row[1][0] === '-' || isNaN(parseFloat(row[1][0])) ? (
                          <div className="odds-value">SUSPENDED</div>
                        ) : (
                          isMatchOdds || title === 'BOOKMAKER'   ? (
                    ...
                    <TableCell>
                      <KhaaiButton
                        className={`${isSelected && tselectedBet.type === (isMatchOdds ? "khaai" : "yes") ? 'selected' : ''} ${
                          !row[2] || !row[2][0] || row[2][0] === '-' || isNaN(parseFloat(row[2][0])) ? 'suspended' : ''
                        }`}
                        onClick={() => {
                          if (!row[2] || !row[2][0] || row[2][0] === '-' || isNaN(parseFloat(row[2][0]))) return;
                          handleBetSelect(
                            rowIndex,
                      ...
                      >
                        {!row[2] || !row[2][0] || row[2][0] === '-' || isNaN(parseFloat(row[2][0])) ? (
                          <div className="odds-value">SUSPENDED</div>
                        ) : (
                          isMatchOdds || title === 'BOOKMAKER'  ? (
